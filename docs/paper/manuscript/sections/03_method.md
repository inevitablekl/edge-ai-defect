...
上述设计形成了分层验证关系：V0 提供 CPU/OpenCV 基线，V2R 建立 CUDA 预处理路径的任务级正确性依据，V3R 在相同 CUDA 预处理语义下进一步隔离主机内存类型。只有先满足相应语义与正确性控制，后续性能差异才可在冻结实验合同内归属于受测路径配置；性能提升本身不能替代正确性验收。由此，性能测试能够在固定模型、固定 Engine、固定推理语义和固定结果合同下比较 3 条路径，而不会把算法修改、量化修改或并发拓扑变化混入同一数据路径消融实验。V3R 的伴随身份验证仍不等同于一次独立的任务级准确率验证。
