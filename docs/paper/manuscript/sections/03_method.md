...
packed BGR图像通过`cudaMemcpy2DAsync`写入持久化设备原始图像缓冲区，随后一个融合CUDA核函数完成resize、padding、BGR→RGB、`1/255`归一化和HWC→NCHW布局转换，直接写入TensorRT管理的FP32 NCHW设备输入。CUDA预处理器、设备缓冲区、同一TensorRT CUDA stream和execution context均跨帧复用，路径关系见图2。
...
三条正式路径均为单帧顺序执行，不使用zero-copy、double buffering、multi-stream、跨帧流水线、显式传输/计算重叠或GPU NMS；这些边界仅限定本文受测实现。

## 2.4 正确性与生命周期控制

三条路径使用相同的冻结180幅测试图像、模型、Engine、输入尺寸、检测阈值和后处理。V2R按预定义任务级差异阈值与V0核验；V3R由同一评价器对冻结预测结果确定性重算，并以张量与检测摘要、帧顺序、处理数量、丢帧、EOS和生命周期检查约束隔离变量身份。该正确性结论仅适用于本文冻结工作负载和评价口径。
