...
| ONNX Runtime C++ SDK version | 1.23.2（Linux x64 CPU，M0.2 依赖接入完成） |
...
| OpenCV | TBD | TBD |
| yaml-cpp | TBD | TBD |
| ONNX Runtime C++ SDK | 1.23.2 | Integrated; API/runtime/provider validation pending M0.3 |
| TensorRT | TBD | TBD |
...
| Next validation | Runtime API version and CPU Provider status in M0.3 |
...
| ONNX Runtime C++ version | 1.23.2，SDK 依赖已在 M0.2 接入；runtime/provider 待 M0.3 验证 |
...
* ONNX Runtime C++ 1.23.2 Linux x64 CPU SDK 依赖已接入；尚未调用 ORT API，runtime version 和 Provider 状态待 M0.3 验证。
