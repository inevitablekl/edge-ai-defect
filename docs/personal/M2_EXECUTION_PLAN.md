...
状态：`CLOSED`（2026-07-18）。M2.0～M2.6 已完成；下一阶段仅可开始 M3
`PostProcessor` preparation，不能把 M2 关闭解释为完整 C++ Serial Baseline、性能
baseline 或 TensorRT 验证完成。
...
5. `test: add M2 validation`
   ...
   - M2.4 已完成：初始化覆盖 missing/non-regular path、SHA256/size、input/output
     name/dtype/shape mismatch；run 覆盖 uninitialized、null output、dtype/layout/
     shape/data-size failure、owned output contract 与 finite test scan。重复
     `initialize()` 明确支持；成功 reinitialize 替换 ready session，失败 reinitialize
     保留之前 ready session，随后 run 继续可用。
   - M2.5 已完成：复用 Level A `frozen_640_checkerboard.f32le` 的冻结
     `float32 NCHW [1,3,640,640]` input，Python 3.10.12 / ONNX Runtime 1.23.2
     生成 raw `BCN [1,10,8400]` golden。production Engine 对同一 raw input 的
     output 在 84000 个元素上得到 `MAE=0`、`max_abs=0`，全部 finite；结果满足
     `MAE <= 1e-6`、`max_abs <= 1e-5`。证据位于
     `results/validation/onnx_runtime_engine_level_b/`，包含 input manifest、Python
     golden、C++ output、comparison report 和 provenance。

6. `docs: close M2 ONNX Runtime engine`
   - 已完成：记录真实验证命令和结果、环境/asset provenance、剩余边界、Git 提交与
     下一阶段说明。

### M2 Gate prerequisites（2026-07-18）

| 项目 | 结果 | 证据 |
| --- | --- | --- |
| Model Smoke OFF | PASS，11/11 CTest | `cmake -S . -B build -DEDGE_AI_ENABLE_MODEL_SMOKE=OFF` 后 build + CTest |
| Model Smoke ON | PASS，18/18 CTest | `EDGE_AI_ENABLE_MODEL_SMOKE=ON`，frozen ONNX path 已设置后 build + CTest |
| strict | Not configured | 当前 `CMakeLists.txt` 没有 strict option/flags，仓库没有现成 strict build directory |
| ASan/UBSan | Not configured | 当前 `CMakeLists.txt` 没有 sanitizer option/flags，仓库没有现成 ASan/UBSan build directory |

上述结果是 M2 Gate 的回归证据。strict 与 ASan/UBSan 都是 `Not configured`，未被
执行，也不得写成已执行通过。
...
production Engine 已验证 CPU synchronous HostTensor in/out、model file integrity、
contract/metadata consistency 与 Level B numerical evidence，并完成本 closeout 文档；
因此 M2 标记为 `CLOSED`。此结论不以 M0 smoke 或单次手工 `Session::Run` 代替 M2
验收，也不将未配置的 strict/ASan/UBSan 伪称为通过。

## 11. M2.6 Closeout（2026-07-18）

### 最终状态

M2：`CLOSED`。阶段名称为 **C++ ONNX Runtime Engine Serial Baseline foundation**。
它提供完整 Serial Baseline 所需的 Engine foundation，不包含 `PostProcessor`、
`SerialRunner`、输入源、结果输出、Profiler 或性能测量。

### 已完成能力

- M2.0：冻结 Engine 范围、CPU execution 语义、`HostTensor` I/O 与 Level B 阈值；
- M2.1：提供 backend-neutral `IInferenceEngine` 和 M2 专用错误表达；
- M2.2：实现 ONNX model integrity、ORT CPU Session 初始化及 contract/metadata
  一致性验证；
- M2.3：实现同步 `HostTensor` → `Ort::Value` → `Session::Run` → owned
  `HostTensor` 输出路径；
- M2.4：覆盖初始化/run 失败路径、transactional output 和 reinitialize 生命周期；
- M2.5：以 Python ORT 1.23.2 与 C++ ORT 1.23.2 对同一固定 `float32 NCHW`
  `[1,3,640,640]` input 完成 raw output Level B；`[1,10,8400]` 的 84000 个元素均
  finite，`MAE=0`、`max_abs=0`，满足冻结阈值。

### 最终回归

| 项目 | 最终结果 | 状态说明 |
| --- | --- | --- |
| Model Smoke OFF | 11/11 CTest PASS | 默认配置的完整回归 |
| Model Smoke ON | 18/18 CTest PASS | 使用 frozen ONNX 资产的完整回归 |
| strict | Not configured | 当前 CMake 没有 strict option/flags 或既有 strict build；未执行 |
| ASan/UBSan | Not configured | 当前 CMake 没有 sanitizer option/flags 或既有 sanitizer build；未执行 |

### Git 提交链

- `568a960 docs: freeze M2 execution plan`
- `5b9c3f9 feat: define inference engine contract`
- `0ea2743 feat: initialize ONNX Runtime session`
- `0ea6b01 feat: implement ONNX Runtime run path`
- `b82878e test: harden ONNX Runtime engine boundaries`
- `e73a0be test: validate ONNX Runtime engine Level B`
- `8f507a4 docs: record M2 gate prerequisites`
- 本 closeout 提交：`docs: close M2 ONNX Runtime engine`

### 未完成能力和下一阶段边界

M2 未实现也未验证：`PostProcessor`、NMS、`Detection`、`SerialRunner`、
`Pipeline`、Profiler、benchmark、TensorRT、CUDA、GPU Execution Provider、ROS2、Qt。
下一阶段建议为 M3 `PostProcessor` preparation；其工作必须消费已验证的 raw
`HostTensor` output，而不能修改 M2 Engine contract 或重新解释 M2 的数值验证。
