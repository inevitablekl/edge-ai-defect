...
Runner 只依赖 `IInferenceEngine`，不依赖 concrete `OnnxRuntimeEngine`；不拥有依赖，所有依赖的生命期
覆盖 Runner；不加载 YAML、解析 CLI、构造 Engine、打开目录或直接写 JSON；严格同步单线程，不修改
M1/M2/M3 合同。
...
- **required contracts**：第 4.3、8.1 节；borrowed lifetime、IInferenceEngine-only、fail-fast、stage context、summary atomicity。
- **required tests**：正常多图、stage order、EOS、Detection order、timing on/off 与 finite/nonnegative；source/preprocess/
...
- **Gate**：必须 Shallow Gate，检查 interface dependency、backend leakage、ownership/lifetime、fail-fast、summary atomicity、
  sink sequence、timing boundary 和 M1/M2/M3 未改义。
...
- **required contracts**：第 7/8.2 节、退出码、metadata 来源、Json→Console composition、relative-only output。
