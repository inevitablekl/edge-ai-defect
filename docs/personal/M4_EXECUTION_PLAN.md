...
M4.4 Shallow Gate pending，M4.5 尚未开始。
...
M4.4 已完成，且没有进入 M4.4 Shallow Gate 或 M4.5。实际新增：
...
IResultSink，覆盖单/多帧、stage/sink 顺序、FrameResult passthrough、Detection order、summary、EOS、timing
开关与 finite/nonnegative、null summary、全部 stage failure、fail-fast、错误上下文与 summary atomicity。它在
构造 Runner 后故意修改外部 TensorInfo，仍验证 Preprocessor 使用构造时 injected value copy；另以无效注入
TensorInfo 证明 preprocess failure 在 engine/sink 前停止。测试不依赖冻结 ONNX 模型，也不构成 Level C。

真实回归：M4.4 runtime 定向 `6/6 PASS`；Model Smoke OFF 全量 CTest `23/23 PASS`；Model Smoke ON 全量
CTest `30/30 PASS`。Strict、ASan、UBSan 仍为 `Not configured`，未运行也未表述为 PASS。下一步必须且仅为
只读 M4.4 Shallow Gate。
