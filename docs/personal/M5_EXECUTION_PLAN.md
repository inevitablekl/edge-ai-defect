...
- **commit**：`test: consolidate M5 validation and baseline evidence`。
- **next**：只读 M5.6。
...
Planning Freeze Remediation 时的阶段状态为：M5.5 Evidence Consolidation `PENDING`、M5.6/M5.7 `PENDING`，
M5 overall `IN_PROGRESS`；该冻结已在后续新的 clean committed HEAD 上完成实际 M5.5 执行，结果见第 25 节。

## 25. M5.5 Evidence Consolidation 实际结果（2026-07-19）

M5.5 已从 clean committed source `c24eefa85bb77543e3267b7aaf797a0d1f02b846` 执行完成。Consolidation Evidence ID
为 `20260719_c24eefa`，正式路径为：

```text
results/consolidation/m5/20260719_c24eefa/
```

目录恰好包含 D040 冻结的六个文件：`README.txt`、`evidence_index.json`、`verification_report.json`、
`provenance.json`、`commands.txt` 和 `sha256sums.txt`；不包含 raw/gzip/TSV、图片、模型、binary 或临时文件。

### 25.1 Consolidation 验证结果

- Git ancestry：Level C source `1073fa8`、Gate 固化 `ea7e0db` 均为 benchmark source `850252b` 的祖先；
  benchmark source 为 M5.4 Evidence commit `703c018` 的祖先；M5.4 提交范围仅为 benchmark Evidence 与两份阶段文档。
- Level C `sha256sum -c`：全部 PASS；16/16、54 detections、per-class `[4,28,7,4,5,6]`，最大 confidence
  error `4.980773571361397e-10`，最大 bbox error `1.2131195092024427e-05`；Python/C++ deterministic。
- Benchmark `sha256sum -c`：36 项全部 PASS；Pilot 100/discard 20/analyzed 80；五个 formal run 均 560 raw、
  50 warmup、510 measured、超过 30 秒且 status `PASS`。
- raw gzip、TSV、per-run summary 和 aggregate 均由已提交 analyzer 重建并 byte-identical；Pilot 未进入 aggregate；
  Type 7、n-1、keep-all measured samples 保持不变。
- ONNX SHA、ModelContract SHA、输入输出 tensor、六类顺序、postprocess 和 ORT CPU SessionOptions 一致；benchmark
  前 12 个 original corpus entry 与 Level C 完全一致，后 8 个为追加 original；D039 资产策略符合。
- 隐私、图片、未压缩 raw、retention 和稳定序列化检查 PASS；formal Evidence 174145 bytes、benchmark Evidence
  492434 bytes、consolidation 22812 bytes，合计 652682 bytes，小于 26214400 bytes。

### 25.2 主要 benchmark across-run 结果
...
结果仍仅表示 WSL2 x86_64 ONNX Runtime CPU Engineering Baseline；不代表 Jetson、TensorRT、最终部署性能或跨硬件
speedup。Consolidation PASS 不等于 M5.6 Deep Evidence Gate PASS。

### 25.3 阶段状态

- M5.4：`COMPLETE`；M5.5 首次 Evidence Consolidation 已生成，但因 provenance completeness 失败而标记为
  `historical_invalidated_consolidation`；
- M5.5 Remediation Planning Freeze：`COMPLETE`；M5.5 Remediation Generation：`COMPLETE`；
- M5.6 Deep Evidence Gate：`PENDING`；M5.7：`PENDING`；
- M5 overall：`IN_PROGRESS`，尚未 `CLOSED`；下一步为只读 M5.6 Deep Evidence Gate Rerun。
...
## 27. M5.5 Consolidation Evidence Remediation Generation 实际结果（2026-07-19）

Planning Freeze 后已从 clean committed source `da86e53d92c62ff03f4fe2aaf6328fb5f051519c` 完成 remediation generation。
新的 active Consolidation Evidence ID 为 `20260719_da86e53`，正式路径为：

```text
results/consolidation/m5/20260719_da86e53/
```

旧 `results/consolidation/m5/20260719_c24eefa/` 保持 `historical_invalidated_consolidation`；本轮未修改、删除或重算其
六文件 SHA。Level C `20260719_1073fa8` 和 Benchmark `20260719_850252b` 均未修改，未生成新的检测或性能样本。

### 27.1 Generation snapshot 与验证阶段

Generation-time Git snapshot 固定为：branch `feature/cpp-onnxruntime`、upstream `origin/feature/cpp-onnxruntime`、
behind `0`、ahead `2`、worktree clean；该 snapshot 不因后续 Git 变化回填 Evidence。

新 consolidation 的 provenance 恰好包含以下 15 条独立记录，顺序固定且全部 `exit_code=0/result=PASS`：
...
gzip_validation
timings_tsv_rebuild
per_run_summary_rebuild
aggregate_summary_rebuild
model_contract_consistency
corpus_consistency
privacy_scan
asset_scan
retention_check
...
`commands.txt` 恰好 15 个编号段，并与 `provenance.command_records` 的 id、phase、command、working directory、exit code、
result 一对一一致。Staging A/B 六文件逐字节一致，双方 `sha256sum -c` 均 PASS；发布后再次 SHA 检查 PASS。

### 27.2 结果与阶段状态

- Strict JSON（duplicate-key rejecting）、schema version 1、字段顺序、UTF-8/LF、隐私和资产检查：PASS；
- ModelContract/ONNX、输入输出合同、ORT CPU、corpus 前 12 项：PASS；
