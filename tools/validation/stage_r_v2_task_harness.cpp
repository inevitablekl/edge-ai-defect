...
void usage() {
    std::cout << "Usage: stage_r_v2_task_harness --config PATH --manifest PATH "
...
int main(int argc, char** argv) {
    ...
    if (config.data_path_variant != edge_ai_defect::runtime::DataPathVariant::kV2 ||
        ...
        std::cerr << "harness requires RuntimeConfig v6 V2 TensorRT INT8 profiling off\n";
        return 3;
    }
    ...
    if (!status.ok()) { std::cerr << "V2 preprocessor: " << status.message() << '\n'; return 4; }
    ...
    for (;;) {
        ...
        if (!status.ok()) { std::cerr << "TensorRT V2 input: " << status.message() << '\n'; return 4; }
