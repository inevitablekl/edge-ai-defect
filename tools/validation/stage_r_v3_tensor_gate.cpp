...
Options parse_options(int argc, char** argv) {
    if (argc != 9) {
        throw GateError("usage: stage_r_v3_tensor_gate --config PATH --manifest PATH "
...
std::vector<CaseInput> load_cases(const fs::path& path) {
    ...
    if (!root["cases"] || !root["cases"].IsSequence() || root["cases"].size() != 16U ||
        ...
        throw GateError("V3 tensor gate requires the frozen 16-case corpus");
    }
    ...
    if (!shape || shape.size() != 4U || shape[0].as<int>() != 1 ||
        ...
        throw GateError("V3 tensor gate requires target shape [1,3,640,640]");
    }
...
void write_report(const Options& options,
    ...
    output << std::setprecision(17)
           << "{\n  \"schema_version\": 1,\n"
           << "  \"validation\": \"stage_r_r2_v3_tensor_gate\",\n"
           ...
           << "  \"validator_mode\": \"V3 pinned raw staging to actual TensorRT input_device copy-back; no enqueue\",\n"
...
int run(const Options& options) {
    ...
    e::stage_r::PinnedRawStaging staging;
    const std::size_t max_total =
        static_cast<std::size_t>(max_height) * static_cast<std::size_t>(max_width) * 3U;
    status = staging.allocate(max_total);
    if (!status.ok()) throw GateError("pinned staging allocation: " + status.message());
    ...
    for (std::size_t i = 0; i < cases.size(); ++i) {
        ...
        if (!status.ok()) throw GateError("pinned staging: " + status.message());
        status = preprocessor->preprocess(staging.data(), staging.width(), staging.height(),
                                          staging.packed_row_bytes(), geometry);
        if (!status.ok()) throw GateError("V3 preprocessing: " + status.message());
    ...
    std::cout << "V3 tensor gate: " << (pass ? "PASS" : "FAIL")
...
int main(int argc, char** argv) {
    try { return run(parse_options(argc, argv)); }
        ...
        std::cerr << "stage_r_v3_tensor_gate: " << error.what() << '\n';
        return 2;
    }
}
